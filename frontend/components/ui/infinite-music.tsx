...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
    ...
    const previousItemRef = useRef<MenuItem | null>(null);
    ...
    // Handle item changes
    useEffect(() => {
        if (activeItem !== previousItemRef.current) {
            if (audioRef.current) {
                audioRef.current.pause();
                setIsPlaying(false);
            }
            previousItemRef.current = activeItem;
        }
    }, [activeItem]);
    ...
    }, [items]); // Remove isPlaying from dependencies to prevent unnecessary re-renders
    ...
    return (
        <div className="relative w-full h-full">
            <canvas id="infinite-grid-menu-canvas" ref={canvasRef} className="overflow-hidden relative w-full h-full outline-none cursor-grab active:cursor-grabbing" />
        ...
        `}
          ...
          ${isMoving
                                ...
                                : 'bottom-[4.15em] opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
                            }
        `}
                    >
